$(document).ready(function(){
    ...
    $('#vm_Limpiar').click(function(){//Limpia el catalogo.
        $('#txt_nombre_motivo_rechazoVM').val('');
        $('#grilla').remove();
    ...
    $('#vm_Buscar').click(function(){//busca �l o los registros
        ...
        //loading('cargando');
        $.post("../modelo/cls_motivo_rechazo.php",{metodo:"cargaGrilla",parametros:creaParametrosvm()},function(data){//se ejecula la b�squeda.
            if(data.filas){//si trae resultado, entra.
                ...
                $.each(data.filas, function(cant_reg,detalle){//se recorre el json.
                    ...
                    var str = detalle.motivo_rechazo_id+",'"+detalle.nombre_motivo_rechazo+"'";
                    //se agrega la fila con sus datos a la grilla.
                    if(detalle.nombre_motivo_rechazo != 'N/D'){
                        $('#grilla tbody').append('<tr id="consulta"><td id="fila" align="left" width="370"><a href="#" onclick="aceptar('+str+')">'+detalle.nombre_motivo_rechazo+'</a></td></tr>');
...
function aceptar(motivo_rechazo_id,nombre_motivo_rechazo){//se carga los datos en formulario que los solicit� al clicar en el registro solicitado y cierra el cat�logo.
    montarMotivoRechazo(motivo_rechazo_id,nombre_motivo_rechazo);
    close_form();
}

function creaParametrosvm(){
    //Retorna los parárametros en notación JSON.
    var objeto = {nombre_genero_musicalVM:$('#txt_nombre_motivo_rechazoVM').val()};
