$(document).ready(function(){						   
    ...
    $('#vm_Buscar').click(function(){//busca �l o los registros
        ...
        //loading('cargando');
        $.post("../modelo/cls_instrumento.php",{metodo:"cargaGrilla",parametros:creaParametrosvm()},function(data){//se ejecula la b�squeda.
            if(data.filas){//si trae resultado, entra.
                ...
                $.each(data.filas, function(cant_reg,detalle){//se recorre el json.
                    ...
                    var str = detalle.instrumento_id+",'"+detalle.nombre_instrumento+"'";
                    //se agrega la fila con sus datos a la grilla.
                    $('#grilla tbody').append('<tr id="consulta"><td id="fila" align="left" width="370"><a href="#" onclick="aceptar('+str+')">'+detalle.nombre_instrumento+'</a></td></tr>');
...
function aceptar(instrumento_id,nombre_instrumento){//se carga los datos en formulario que los solicit� al clicar en el registro solicitado y cierra el cat�logo.
    montarInstrumento(instrumento_id,nombre_instrumento);
    close_form();
}

function creaParametrosvm(){
    //Retorna los parárametros en notación JSON.
    var objeto = {nombre_instrumentoVM:$('#txt_instrumentoVM').val()};
